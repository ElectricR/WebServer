...
def put_req(target, data, token=None):
    ...
    return req.put(f'http://localhost:{os.environ["BALANCER_PORT"]}/{target}',
                   data=data,
                   headers=headers)
...
def get_req(target, token=None):
    ...
    if token:
        headers['Authorization'] = f'Bearer {token}'
    return req.get(
        f'http://localhost:{os.environ["BALANCER_PORT"]}/{target}',
        headers=headers)
...
def auth(login, password):
    ...
    return req.get(
        f'http://localhost:{os.environ["BALANCER_PORT"]}/auth',
        headers=headers)
...
class integration_test(unittest.TestCase):
    ...
    def test_deletion(self):
        ...
        self.assertEqual(
            put_req("file1", "{1:2}",
                    token=self.killer_token).status_code, 201)
        self.assertEqual(
            put_req("file2", '{"Alice":"Bob"}',
                    token=self.killer_token).status_code, 201)

        self.assertEqual(
            get_req("file1", token=self.killer_token).text, "{1:2}")
        self.assertEqual(
            delete_req("file1", token=self.killer_token).status_code, 204)
        self.assertEqual(
            get_req("file1", token=self.killer_token).status_code, 404)
        self.assertEqual(
            delete_req("file1", token=self.killer_token).status_code, 204)

        self.assertEqual(
            get_req("file2",
                    token=self.killer_token).text, '{"Alice":"Bob"}')
        self.assertEqual(
            delete_req("file2", token=self.killer_token).status_code, 204)
        self.assertEqual(
            get_req("file2", token=self.killer_token).status_code, 404)
    ...
    def test_rewrite_put(self):
        ...
        self.assertEqual(
            get_req("file1", token=self.killer_token).text, "{2:1}")

        self.assertEqual(
            put_req("file1", '{"Apple":"Orange"}',
                    token=self.killer_token).status_code, 201)

        self.assertEqual(
            get_req("file1",
                    token=self.killer_token).text, '{"Apple":"Orange"}')

        self.assertEqual(
            put_req("file1", '{"Apple":100}',
                    token=self.killer_token).status_code, 201)

        self.assertEqual(
            get_req("file1",
                    token=self.killer_token).text, '{"Apple":100}')
