...
def get_url(file_name):
    ...
    return 'http://{0}:{1}/storage/{2}'.format(
        f'pm-default-server-container-{str(instance)}',
        f'{int(os.environ["PORT"]) + instance}', file_name)


def get_wait_url():
    return 'http://{0}:{1}/wait'.format(
        'pm-default-server-container-1',
        f'{int(os.environ["PORT"]) + 1}')
...
class Balancer:
    ...
    def check_JWT_auth(self, auth):
        ...
        if self.usr_tokens[j['name']] == auth.encode() \
                and j['role'] == 'write':
            return True
        else:
            return False
    ...
    def wait_req(self):
        try:
            self.check_JWT_auth(request.headers.get(
